...
# 3. Add libraries: FirebaseESP32, ArduinoJson, Time
# 4. Start simulation
...
// Pin Configuration (Wokwi virtual pins)
#define DHT_PIN 4
#define FAN_RELAY 26
...
void setup() {
  Serial.begin(115200);
  Serial.println("=== WOKWI ESP32 FIREBASE SIMULATOR ===");
  ...
  // Connect to WiFi (Wokwi built-in)
  connectWiFi();
  ...
  Serial.println("Open Serial Monitor to see Firebase connection");
  Serial.println("Check Firebase console for real-time data");
}
...
void streamCallback(StreamData data) {
  ...
  Serial.println("🎛️ Control changed: " + path + " = " + value);
...
void uploadSensorData() {
  ...
  if (Firebase.setJSON(firebaseData, path.c_str(), json)) {
    ...
    Serial.print("🌡️ Temp: "); Serial.print(sensors.temperature); Serial.print("°C, ");
    ...
    Serial.print("☀️ Light: "); Serial.print(sensors.lightLevel); Serial.println("%");
